...
if __name__ == "__main__":
    ...
    for dept in DEPARTMENTS:
        ...
        for course in getCoursesInDept(dept):
            cur.execute('''INSERT INTO classes(Class,Course,Title,Section,Type,Days,Time,Room,Location,Instructors,Status)
                VALUES (?,?,?,?,?,?,?,?,?,?,?)''',(course['Class'],course['Course'],course['Title'],course['Section'],course['Type'],course['Days'],course['Time'],course['Room'],course['Location'],course['Instructor (*) additional instructors'],course['Status']))
        conn.execute("commit")
